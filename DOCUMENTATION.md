...
**Meridian** delivers **local-first, weather-aware irrigation intelligence** for Hubitat — combining **real-time evapotranspiration (ET)**, **seasonal water budgeting**, and **optional full-program scheduling**.
...
Rain Bird, Rachio, Hunter, Hydrawise, and of course Meridian, as well as practically all “smart” controllers, use this formula:
...
Every day, Meridian fuses live data from your selected provider — **NOAA**, **OpenWeather**, **Tomorrow. io**, or **Tempest PWS** — to compute:
...
If ET is 30% above normal, Meridian increases watering time 30%.
...
Meridian includes three primary configuration pages — **Zone Setup**, **Soil Settings**, and **Scheduling** — which define the foundation of irrigation behavior.
...
Meridian brings **full irrigation control** to your Hubitat hub — while still serving as a data engine for external automations.
...
Every Meridian installation supports up to **16 independent programs**, each with:
...
Meridian supports voice control through Hubitat's built-in **Amazon Echo Skill** by exposing a virtual Echo child device for each irrigation program.

When enabled for a program:

- A dedicated **Meridian Echo** child device is created
...
- Respecting Hubitat's security model, Meridian cannot automatically determine if you have the **Amazon Echo Skill** enabled.
...
- An optional `Meridian` prefix can be enabled per program to avoid name collisions with other smart devices
...
Meridian automatically distributes the runtime across cycles.
...
Once Base Time and Cycle & Soak values are configured, Meridian handles the rest automatically.
...
Before each program runs, Meridian evaluates local conditions from your selected weather provider(s):
...
All skip events are logged and reflected in the *Meridian Data* driver attributes (`freezeAlert`, `rainAlert`, `windAlert`, etc.).
...
Meridian automatically clamps runtimes to valid limits and logs all computations for transparency.

---

### 🧠 Smart Sequencing & Conflict Detection

Meridian detects overlapping program schedules automatically.
...
Even with the internal scheduler active, Meridian continues to function as a **data provider** for custom controllers, dashboards, and integrations.
All computed ET, seasonal, alert, and summary data are published to the **Meridian Data** child driver as both:
...
This architecture keeps Meridian fully compatible with both **automation frameworks** (Rule Machine, Node-RED, webCoRE) and **fully autonomous scheduling** — one engine, two use cases.
...
Meridian adds a unique ability — to **“end by” sunrise** — automatically back-calculating when to start so watering finishes right as daylight begins.
...
> 🕐 *“Meridian doesn’t just know when to start watering — it knows when you want it to finish.”*
...
- Keep the **Meridian Data** driver installed even if scheduling is disabled — it remains the data backbone for dashboards and custom automations.

---

## 🧩 Sunrise/Sunset Automation Templates

Meridian started as a data supplier-only project; keeping that in mind, it still supplies real-time **ET budgets** and **timestamps** that can be combined with sunrise/sunset logic in Rule Machine, webCoRE, or Node-RED.
...
**Action Sequence:**
```groovy
Set Variable wetitData = %device:Meridian Data: datasetJson%
...
![Piston Example](https://github.com/MHedish/Hubitat/blob/main/Apps/Meridian/images/WebCoRE.png)
...
- Hubitat Device Node → `Meridian Data`
...
1. 02:00 → Meridian updates weather (`wxChecked`, `wxTimestamp`)
2. Sunrise → Rule Machine/webCoRE trigger runs irrigation
3. Runtime scaled by ET percentage (`etBudgetPct`)
4. Controller marks completion → Meridian resets soil depletion
...
Meridian integrates multiple weather data sources to provide accurate, redundant inputs for **Evapotranspiration (ET)**, **forecast modeling**, and **alert logic**.
Users can select their preferred provider, or Meridian can automatically fall back to another when conditions or data gaps are detected.
...
---
## Why Meridian does **not** cascade across multiple weather providers
...
Meridian intentionally avoids cascading through multiple weather providers within a single update cycle. Once a provider is selected for a given forecast window, it remains authoritative for that window.
...
If Meridian were to cascade freely:
...
## Design choice in Meridian

Therefore, Meridian follows these rules:
...
When multiple providers are configured, Meridian dynamically merges data to create a **hybrid local model**:
...
2. **Tomorrow. io / Open-Meteo, OpenWeather (Forecast Layer):**
   ...
   Meridian uses this layer for *skip-ahead* logic (rain prediction).
3. **NOAA NWS (Fallback & Validation):**
   Provides regional consistency and baseline validation for forecast data.

If a data source becomes unavailable, Meridian automatically reverts to the next available provider without interrupting scheduled operations.
...
Meridian continuously publishes weather conditions and alert states to the **Meridian Data** driver, making them available for dashboards, automations, and notifications.
...
Meridian is fully location-aware. All weather forecasting, evapotranspiration (ET), seasonal modeling, and soil calculations are **explicitly dependent upon geographic location**.

### How Location Is Determined

- Meridian uses the hub’s configured latitude and longitude as the authoritative source.
...
- Meridian logs the previous and current coordinates
- Regional data (weather source eligibility, ISO subdivision, etc.) is refreshed
- If soil memory is enabled, Meridian will warn that existing soil depletion data may no longer be valid and recommend a manual reset.

This is to ensure ET and irrigation decisions remain scientifically consistent after relocation.

### Weather & ET Dependency

All weather forecasts used by Meridian (NOAA, Open-Meteo, OpenWeather, Tomorrow.io, Tempest) are **location-specific** and directly influence
...
Meridian may perform one-time reverse geocoding to determine regional data availability and jurisdictional context.  Reverse geocoding is provided by Geoapify. Map data © OpenStreetMap contributors.
...
- **Automatic Refresh:** Meridian updates all weather data daily (default 02:00 local).
...
- Meridian automatically aligns Tempest device data with Hubitat’s event timeline, ensuring synchronized driver updates.
...
Even if you’re using Meridian only as a **data provider**, you can leverage these attributes to build automations such as:
...
If your selected provider is unavailable, Meridian automatically retries using NOAA (when the option is enabled).
...
The **Meridian Data Driver** exposes a complete set of attributes and commands that mirror the app’s internal logic.
...
Each Meridian installation supports up to **48 zones**, with the following attributes automatically generated:
...
- Commands can be run directly from the **Meridian Data** driver page or invoked through **Rule Machine**, **webCoRE**, or the **Maker API**.
...
The Meridian parent app manages all configuration parameters that define how the irrigation model operates.
...
Meridian integrates with the Hubitat **Notifications** app to provide real-time alerts about irrigation activity, weather conditions, and zone operation.

Meridian does **not send notifications directly**. Instead, it emits events from its data child device which the Notifications app can subscribe to and route to notification devices such as:
...
This design keeps Meridian focused on irrigation logic while allowing Hubitat to manage message delivery, rate limiting, and scheduling.

---

### Notification Event Model

Meridian uses a **button-based event model** for notifications.

Each event category is represented by a button number on the Meridian data child device.
...
To receive Meridian notifications:

1. Open the **Hubitat Notifications** app.
2. Select the **Meridian Data Device** as a trigger device.
...
The **Test Notifications** button in the **Logging & Tools** section sends a test alert through the Meridian notification system.
...
The test message uses the same event mechanism as normal Meridian notifications.

---

### Why Meridian Uses This Design
...
Because Meridian only emits events, the notification system remains flexible and easy to customize.
...
Each irrigation **Zone** in Meridian represents a physical valve or watering area such as turf, garden beds, shrubs, or trees.
...
Meridian supports both **`capability.valve`** and **`capability.switch`** devices for each irrigation zone.
...
- Tap **“Stop”** to end the test; Meridian records the elapsed time for accurate ET adjustments.
...
When the app runs, each zone publishes real-time data to the **Meridian Data Driver** using the following attributes:
...
When balance drops below the MAD threshold, Meridian automatically schedules watering or flags the zone for manual attention.
...
The **Soil Memory System** is one of Meridian’s most powerful features, providing a realistic simulation of how water behaves in your soil between irrigation events.
...
The balance is expressed as a percentage of available soil water (0–100 %), and when it falls below the **Management Allowed Depletion (MAD)** threshold, Meridian triggers irrigation to replenish it.
...
| Adjusted Runtime | seconds | Calculated automatically by Meridian based on ET and seasonal adjustments. | Displayed in the Data Driver as `zoneXAdjustedTime`. |
...
Up to **16 independent programs** can be defined per Meridian instance.
...
Meridian’s scheduler can begin watering **at sunrise** or calculate a start time so the program **ends by sunrise**.
...
Meridian can automatically determine soil type for zones in the United States using the USDA Soil Survey.

**How it works:**
1. App uses hub's latitude/longitude
2. Queries USDA Soil Data Access (SDA) API
3. Returns soil texture and hydrologic group
4. Maps USDA texture to Meridian soil types
...
The summary of each program execution is also published to the **Meridian Data Driver** under the `summaryText` attribute.
...
Meridian’s weather and alert system enables dynamic watering decisions based on **forecast**, **live data**, and **safety conditions** such as freezing or wind.
...
- Publish `*_Alert` attributes in the **Meridian Data Driver**.
...
This order ensures that Meridian prioritizes **local and current** conditions over regional forecasts.
...
└─────────────┘    └──────────────┘    └───────────────┘    └────────────────┘
       ▲                                                     │
       │<─────────── Meridian calculates ET since last mark ─┘
```
...
ET calculations are **time-based**, not daily resets. Meridian determines soil depletion by measuring how long it’s been since watering — making accurate resets essential for realistic modeling.

---

## 🌧 Rain Protection Logic<a id="-rain-protection-logic"></a>

Meridian provides multiple layers of **rain protection** to prevent unnecessary irrigation during or before rainfall.
...
Meridian automatically monitors **forecasted and current wind speeds** and will skip irrigation programs when conditions exceed a user-defined threshold.
...
Meridian’s wind protection logic mitigates this by continuously analyzing forecast wind speeds from your active weather provider.
...
When wind protection is active, the Meridian Data driver publishes:
| Attribute | Type | Description |
|:--|:--|:--|
...
Meridian consolidates **Freeze**, **Rain**, and **Wind** alert data into a single **Active Weather Alerts** panel within the app UI.
...
If multiple alerts are active simultaneously, Meridian applies a deterministic priority system to prevent overlap conflicts:
...
3. Confirm that the `windAlert` and `windSpeed` attributes in the **Meridian Data** driver match current conditions.
...
Meridian monitors forecast wind values.
...
Meridian continuously publishes both **summary** and **per-zone data** to its child device — the **Meridian Data driver**.
...
> **Meridian — bringing data-driven irrigation to life through meteorology, soil science, and Hubitat automation.**